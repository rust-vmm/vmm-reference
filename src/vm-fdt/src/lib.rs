...
mod tests {
    ...
    // Comparing arrays with len > 32 is not possible with rust stable.
    // This is a workaround until Rust allows for larger array checks as well.
    ...
    fn fdt_compare(expected_fdt: &[u8], actual_fdt: &[u8]) {
        ...
        let group_of_32 = f64::ceil(actual_fdt.len() as f64 / 32f64) as usize;
        for g in 0..group_of_32 {
            let end = usize::min((g + 1) * 32, actual_fdt.len());
